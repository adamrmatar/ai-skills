# Claude Code Custom Instructions - Automating Email Responses And Meeting Scheduling With Openclaw
...
1. **Enable Two-Factor Authentication**: Go to your Google account settings and enable two-factor authentication.
2. **Generate App Password**: Navigate to the App Passwords section and generate a new password for OpenClaw.
3. **Use Password**: Use the generated password to connect OpenClaw to your Gmail account.
...
1. **Enable API**: Go to the Google Cloud Console and enable the Google Calendar API.
2. **Create OAuth Client ID**: Create an OAuth client ID for your application.
3. **Authenticate**: Follow the authentication steps to grant OpenClaw access to your Google Calendar.
...
1. **Choose a Plan**: Select a VPS plan that suits your needs (e.g., KVM 2).
2. **Apply Discount**: Use any available coupon codes for discounts.
3. **Complete Setup**: Provide necessary details and payment information to complete the setup.
4. **Access VPS**: Once set up, access your VPS through the provided interface.
...
1. **Create a Bot**: Search for BotFather in Telegram and use the `/newbot` command to create a new bot.
2. **Name Your Bot**: Provide a name and username for your bot.
3. **Get Token**: BotFather will provide an access token. Use this token to link your bot to OpenClaw.
